...
#---------------change ------------------------
    def _convert2sequence(self,atac):
        ...
        path='/home/ubuntu0/GLUE/experiments/Atlas/scripts/mm10.fa'
        seq2 = bindome.tl.get_sequences_from_bed(atac.var[['chrom', 'chromStart', 'chromEnd']],
        ...
        #find max length of chrom
        global chrom
        chrom=len(max(seq_dict.values(),key=len))
        self.chrom=chrom
       

        # need to pad the sequence in order to have the same length
        atac_seq = self.onehot_mononuc_multi(seq_dict, chrom)
    ...
    def onehot_mononuc_multi(self, seqs, max_length):
        ...
        for i, seq in seqs.items():
            ...
            else:# to allow truncation for chrom length larger than the median
    ...
    def _extract_data_train(self, data_configs: List[DATA_CONFIG]) -> Tuple[
        ...
        xatac = [self._extract_atac_x(adata, data_config)
                 for adata, data_config in zip(self.adatas, data_configs)]
        xbch = [
        ...
        return xuid, (x, xrep,xatac, xbch, xlbl, xdwt)
    ...
    def _extract_data_eval(self, data_configs: List[DATA_CONFIG]) -> Tuple[
        ...
        return xuid, (x, xrep,xatac,xbch, xlbl, xdwt)
#----------------------change in extract_x------------------------------
    def _extract_atac_x(self, adata: AnnData, data_config: DATA_CONFIG)-> AnyArray:
        default_dtype = get_default_numpy_dtype()
