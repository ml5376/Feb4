...
class SeqDataEncoder(glue.DataEncoder):
    def __init__(
        ...
        ptr_dim = 4
        in_dim = chrom_len // 3 // 2 // 3 // 2 // 3 //2 
        setattr(self, f"sequntial_{0}", torch.nn.Sequential(
            #b1
            torch.nn.Conv1d(ptr_dim, 288, kernel_size=17, stride=1, padding=17 // 2), torch.nn.ReLU(), # 1344,28
            torch.nn.MaxPool1d(kernel_size=3),  # 1344//3,28

            torch.nn.Conv1d(288, 288, kernel_size=5, padding=5 // 2),   # 1344//3,28
            torch.nn.MaxPool1d(kernel_size=2),  # 1344//3//2,28
            #b2
            torch.nn.Conv1d(288, 288, kernel_size=17, stride=1, padding=17 // 2),  # 1344,28
            ...
            torch.nn.Conv1d(288, h_dim, kernel_size=1),  # 1344//3//2,h_dim
            
            torch.nn.Flatten(),
            torch.nn.Linear(in_dim*h_dim, h_dim) 
    ...
    def forward(  # pylint: disable=arguments-differ
        ...
        ptr = getattr(self, f"sequntial_{0}")(ptr)
