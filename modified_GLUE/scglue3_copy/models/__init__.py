...
def fit_SCGLUE(
    ...
    atac=adatas['atac']
    chrom = convert2sequence(atac[:,atac.var.query('highly_variable').index.to_numpy().tolist()])

    pretrain = model(chrom,adatas,sorted(graph.nodes), **pretrain_init_kws)
    ...
    if "directory" in pretrain_fit_kws:
        pretrain.save(os.path.join(pretrain_fit_kws["directory"], "pretrain_glue3.dill"))
    ...
    if "directory" in finetune_fit_kws:
        finetune.save(os.path.join(finetune_fit_kws["directory"], "fine-tune_glue3.dill"))
