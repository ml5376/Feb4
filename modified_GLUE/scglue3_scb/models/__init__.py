...
def fit_SCGLUE(
    ...
    atac=adatas['atac']
    #using highly variable features 
    atac_hvf=atac[:,atac.var.query('highly_variable').index.to_numpy().tolist()]
    chrom,atac_seq = convert2sequence(atac_hvf)

    pretrain = model(atac_seq,adatas,sorted(graph.nodes), **pretrain_init_kws)
    
    pretrain.compile(**compile_kws)

    pretrain.trainer.atac=atac_hvf
    # pretrain.trainer.seqdata=torch.as_tensor(atac_seq,dtype=torch.float32).to(device=pretrain.trainer.net.device,non_blocking=True)
    
    print('pretrain.fit---------------------')
    # pretrain = load_model("/home/ubuntu0/GLUE/glue/pretrain/pretrain_scb2.dill")
    # pretrain.load_state_dict("/home/ubuntu0/GLUE/glue/pretrain/checkpoint_353.pt")# no such function
    pretrain.fit(adatas, graph, **pretrain_fit_kws)#SCGLUEModel.fit

    if "directory" in pretrain_fit_kws:
        pretrain.save(os.path.join(pretrain_fit_kws["directory"], "pretrainscb.dill"))

    fit_SCGLUE.logger.info("Estimating balancing weight...")
    for k, adata in adatas.items():
        print('pretrain.encode_data')
        adata.obsm[f"X_{config.TMP_PREFIX}"] = pretrain.encode_data(k, adata,torch.as_tensor(atac_seq,dtype=torch.float32)
    )
        print('embedding size: ',k,adata.obsm[f"X_{config.TMP_PREFIX}"].shape)
    ...
    if "directory" in finetune_fit_kws:
        finetune_fit_kws["directory"] = \
            os.path.join(finetune_fit_kws["directory"], "fine-tunescb")

    finetune = model(atac_seq,adatas, sorted(graph.nodes), **init_kws)
    
    finetune.adopt_pretrained_model(pretrain)
    finetune.compile(**compile_kws)

    finetune.trainer.atac=atac_hvf
    # finetune.trainer.seqdata=torch.as_tensor(atac_seq,dtype=torch.float32).to(device=finetune.trainer.net.device,non_blocking=True)
    ...
    if "directory" in finetune_fit_kws:
        finetune.save(os.path.join(finetune_fit_kws["directory"], "fine-tunescb.dill"))
