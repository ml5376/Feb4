...
from ..utils import AUTO, config, get_chained_attr, logged,convert2sequence
from . import sc
...
class SCGLUETrainer(GLUETrainer):
    ...
    **kwargs
    ...
    def get_first_batch_size(self,u,iter,batch_size):
        size=config.DATALOADER_FETCHES_PER_BATCH
        # print('atac batch size',size)
        return u[iter*batch_size:(iter+1)*batch_size,:]
    
    def subsample_shuffle_idx(self,u,iter,batch_size):
        #get shuffle idx match with xatac
        # u=u[idx_list,:]
        self.u=u.clone()
        return self.get_first_batch_size(u,iter,batch_size)

    def compute_losses(
            self, data: DataTensors,epoch,dsc_only: bool = False
        ...
        for k in net.keys:
            if k=='rna':
                u[k], l[k],locrna = net.x2u[k](x[k], xrep[k], lazy_normalizer=dsc_only)
                
            else:# if k is 'atac'
                data_size = self.shuffle_idx.shape[0]//(config.DATALOADER_FETCHES_PER_BATCH*self.getitem_size)
                iter=self.iter-data_size*(self.epoch-1)
                iter=iter-1
                # print('iter:    ',iter,self.prev_epoch,self.epoch,self.shuffle_idx.shape)
                idx = self.shuffle_idx[:,1]
                if self.epoch>self.prev_epoch:# forward_atac called at the start of epoch 
                    self.feature_index=torch.randperm(self.atac.shape[1]).numpy()
                    _,seqdata=convert2sequence(self.atac[:,self.feature_index.tolist()])# shuffle feature on input features 
                    seqdata=torch.as_tensor(np.array(seqdata),dtype=torch.float32).to(device=self.net.device,non_blocking=True)
                    self.countdata=torch.as_tensor(self.atac[:,self.feature_index.tolist()].X.T.toarray(),dtype=torch.float32).to(device=self.net.device,non_blocking = True)
            
                    self.prev_epoch=self.epoch
                    if self.epoch==1:
                        lsi=torch.as_tensor(self.atac.obsm['X_lsi'],dtype=torch.float32).to(device=self.net.device,non_blocking=True)
                        self.net.x2u.atac.sequntial_0[9].weight=torch.nn.Parameter(lsi)#pass weight

                    ptr=net.x2u[k](seqdata,xatac[k],xrep[k],lazy_normalizer=dsc_only)#forward 
                   
                    self.loc=self.net.x2u.atac.sequntial_0[9].weight[idx]#ncell,100
                   
                    self.ptr=ptr.data
                    size=locrna.shape[0]
                    loc= self.subsample_shuffle_idx(self.loc,iter,size)
                
                else:
                    size=locrna.shape[0]
                    loc= self.get_first_batch_size(self.loc,iter,size)
            
                u[k],l[k],_=net.x2u['rna'](xrep[k], loc, lazy_normalizer=dsc_only)
         
               
                
        #define scbasset loss; use self.loc and atac count 
        scb_loss={}
        scb_loss_fn=torch.nn.BCEWithLogitsLoss()
        scb_loss['atac']=scb_loss_fn(self.ptr,self.countdata)
        scb_loss['rna']=0
        ...
        if anneal:
            ...
            u_cat = u_cat.clone() + (anneal * self.BURNIN_NOISE_EXAG) * noise
        ...
        x_nll = {
            k: -net.u2x[k](
                usamp[k], vsamp[getattr(net, f"{k}_idx")], xbch[k], l[k],k
            ).log_prob(xatac[k]).mean()
            for k in net.keys
        }
        x_kl = {
            k: D.kl_divergence(
        ...
        x_elbo = {
            k: x_nll[k] + self.lam_kl * x_kl[k] + scb_loss[k]
            for k in net.keys
        }
    ...
    def train_step(
        ...
        self.epoch=epoch
        self.iter=engine.state.iteration #the first iteration is 1
        ...
        if self.freeze_u:
            ...
            with torch.autograd.detect_anomaly():
                losses["dsc_loss"].backward(retain_graph=True)  # Already scaled by lam_align
        ...
        self.net.zero_grad(set_to_none=True)
        with torch.autograd.detect_anomaly():
            losses["gen_loss"].backward(retain_graph=True)
...
class SCGLUEModel(Model):
    ...
    def __init__(
            self, atac_seq, adatas: Mapping[str, AnnData],
        ...
        for k, adata in adatas.items():
            ...
            if k == 'atac':
                atac=adata[:,adata.var.query('highly_variable').index.to_numpy().tolist()]
                x2u[k] = _ENCODER_MAP[data_config["prob_model"]](chrom,
                    atac.shape[1],100, adata.shape[0],
                    h_depth=h_depth, h_dim=h_dim, dropout=dropout
                )
    ...
    @torch.no_grad()
    def encode_data(
            self, key: str, adata: AnnData, seqdata, batch_size: int = 128,
        ...
        for iter,data in enumerate(data_loader):
            ...
            if key == 'rna':
                ...
                if iter==0:# iter in enumerate start with 0 
                    ptr = encoder(
                        seqdata.to(self.net.device, non_blocking=True),
                        xatac.to(self.net.device, non_blocking=True),
                        xrep.to(self.net.device, non_blocking=True),
                        lazy_normalizer=True
                    )# if don't use subsampling 
                    self.loc=encoder.sequntial_0[9].weight[idx].squeeze()
             
                    loc = self.subsample_shuffle_idx(self.loc,iter,batch_size)
                   
                else:
                    loc = self.get_first_batch_size(self.loc,iter,batch_size)
                    
                
                u=self.net.x2u['rna'](xrep,loc,lazy_normalizer=True)[0]
