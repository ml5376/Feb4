...
class SeqDataEncoder(glue.DataEncoder):
    def __init__(
            self, chrom_len,in_features: int,latent_dim: int,out_features: int,
        ...
        ptr_dim = 4
        in_dim = chrom_len // 3 // 3 #// 3 // 2
        setattr(self, f"sequntial_{0}", torch.nn.Sequential(
            torch.nn.Conv1d(ptr_dim, 4, kernel_size=17, stride=1, padding=17 // 2), torch.nn.Sigmoid(),  # 1344,28
            torch.nn.MaxPool1d(kernel_size=3),  # 1344//3,28

            torch.nn.Conv1d(4, 4, kernel_size=5, padding=5 // 2), torch.nn.Sigmoid(),  # 1344//3,28
            torch.nn.MaxPool1d(kernel_size=3),  # 1344//3//2,28

            torch.nn.Conv1d(4, 1, kernel_size=1),  # 1344//3//2,h_dim
            torch.nn.Flatten(),
            
            # torch.nn.Linear(in_dim, out_features)
        ))# the part of the code to simulate scbasset procedure
        self.loc = torch.nn.Linear(in_dim, out_features)
        self.std_lin = torch.nn.Linear(in_dim,out_features)
        self.loc2=torch.nn.Linear(in_features,latent_dim)
        self.std_lin2=torch.nn.Linear(in_features,latent_dim)
    ...
    def forward(  # pylint: disable=arguments-differ
            self, x: torch.Tensor, xatac: torch.Tensor,xrep: torch.Tensor,
        ...
        if xrep.numel():
            ...
            ptr = xrep
            print('Error: atac has embeddings')
        else:
            ptr = x #sequence
            l = self.compute_l(xatac) # l is needed for nll

            # ptr = self.normalize(x, l)
        ptr = getattr(self, f"sequntial_{0}")(ptr).clone()
        # loc = self.loc(ptr).permute(1,0)
        loc = self.loc(ptr)
        loc = loc.T.data
     
        std = F.softplus(self.std_lin2(self.std_lin(ptr).clone().permute(1,0).data)) + EPS
        loc = self.loc2(loc)
