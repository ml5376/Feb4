        ...
        for k in net.keys:
            if k=='rna':
                ...
                if self.epoch>self.prev_epoch:# forward_atac called at the start of epoch 
                    ...
                    loc,std,l[k]=net.x2u[k](self.seqdata,xatac[k],xrep[k],lazy_normalizer=dsc_only)
                    self.l = l[k]
                    self.loc=loc[idx]
                    self.std=std[idx]
                    ...
                    std= self.subsample_shuffle_idx(self.std,iter,size)
                    
                else:
                    size=locrna.shape[0]
                    loc= self.get_first_batch_size(self.loc,iter,size)
                    std= self.get_first_batch_size(self.std,iter,size)
                    l[k]=self.l #try to use a constant l 
                   
               
                u[k]=D.Normal(loc, std)
        ...
        for k, adata in adatas.items():
            ...
            if k == 'atac':
                ...
                x2u[k] = _ENCODER_MAP[data_config["prob_model"]](chrom,
                    atac.shape[1],latent_dim, adata.shape[0],
                    h_depth=h_depth, h_dim=h_dim, dropout=dropout
                )
        ...
        for iter,data in enumerate(data_loader):
            ...
            if key == 'rna':
                ...
                if iter==0:# iter in enumerate start with 0 
                    ...
                    self.loc = loc[idx].squeeze()
                    self.std = std[idx].squeeze()
                    loc = self.subsample_shuffle_idx(self.loc,iter,batch_size)
                    std = self.subsample_shuffle_idx(self.std,iter,batch_size)
                else:
                    loc = self.get_first_batch_size(self.loc,iter,batch_size)
                    std = self.get_first_batch_size(self.std,iter,batch_size)
                u = D.Normal(loc,std)
